    ...
    >
      <div class="container mx-auto flex justify-between items-center py-[12px] lg:py-[24px]">
        ...
        <div class="flex items-center gap-[12px] lg:gap-[24px]">
          <img
            class="w-[32px] lg:w-[48px] aspect-square"
            :alt="siteConfig.displayName"
            :src="siteConfig.logo"
          />
          <div class="flex items-center gap-[8px] lg:gap-[12px]">
            <h2 class="text-[28px] lg:text-[36px]">{{ siteConfig.displayName }}</h2>
            <!-- 整合的網站切換器 -->
            <div class="flex flex-col gap-[4px] lg:gap-[6px]">
              <button
                ...
                :class="[
                  'px-[8px] lg:px-[12px] py-[4px] lg:py-[6px] text-[12px] lg:text-[14px] font-medium rounded-[6px] transition-all duration-200',
...
import { resetApiInstances } from '@/composables/axios'
...
const siteConfig = computed(() => ({
  ...
  displayName: currentSite.value === 'yenshow' ? '遠岫科技' : '蝶蛹科技',
}))

// 網站切換功能
const switchSite = async (siteKey) => {
  // 設置新的網站
  siteStore.setSite(siteKey)

  // 重置 API 實例，以便使用新的 baseURL
  resetApiInstances()
